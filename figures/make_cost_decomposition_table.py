...
def make_table(rows: list[dict]) -> str:
    ...
    lines = [
        r"\begin{table}[!ht]",
        r"  \centering",
        r"  \caption{Full-model YOLOv8n cost decomposition in time (ms) and",
        r"  percentage on RTX~3080. Cost is classified into worklist construction",
        r"  (Cons.), convolution (Conv.), elementwise operators (Elem.), and other",
        r"  cost.}",
        r"  \Description{A table divides every YOLOv8n frame into low- and",
        r"  high-activity halves and compares full-model latency attributed to",
    ...
    lines.extend(
        [
            ...
            r"  all frames from the evaluated MOT16 test sequences at the median",
            r"  per-frame active ratio; their parenthesized labels report bin means.",
            r"  We obtain each stage time by multiplying its per-frame NSYS fraction",
            r"  by the corresponding CUDA-event latency from the end-to-end",
            r"  evaluation. \textemdash{}",
            r"  denotes not applicable because only WISEConv",
            r"  constructs an explicit worklist.}",
            r"\end{table}",
            "",
        ]
    )
