...
by the mask policy, the propagated output mask $M_{out}$, and a worklist $\mathbf{q}$
...
\begin{enumerate}
  \item \textbf{Construction stage.} A mask-space kernel evaluates
        Eq.~\eqref{eq:mask}, writes $M_{out}$, and compacts its active coordinates
...
Eq.~\eqref{eq:mask} and writes the result to $M_{out}[p]$. It then selects the active
positions within that tile:
\begin{equation}
  \mathcal{I}_n = \{j : M_{out}[T_n[j]]=1\},
  \label{eq:support}
\end{equation}
...
\FOR{each tile $T_n$ \textbf{in parallel}}
  \FOR{each position $p\in T_n$}
    \STATE $M_{out}[p]\leftarrow$ Eq.~\eqref{eq:mask}
  \ENDFOR
  \STATE $\mathbf{q}_n\leftarrow(T_n[j])_{j:M_{out}[T_n[j]]=1}$
