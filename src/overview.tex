...
the critical path. \sysname instead reads the exact worklist
length inside the compute kernel, looks up an offline-calibrated table keyed by
the current length, and decomposes the workload accordingly to keep the GPU's
processing capacity occupied. This on-device
adaptation sustains GPU throughput as the worklist length changes from frame to
frame without any host-side decision on the per-frame path.
