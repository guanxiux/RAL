...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/wiseconv_overview.png}
  \caption{Overview of \sysname and baseline execution paths on a
  two-streaming-multiprocessor (SM) example.
  (a)~Masked convolution: $M_{out}$ marks active output positions
  (colored); the operator computes only these from $\mathbf{x}$ and
  $\mathbf{f}$.
  (b)~Tile skipping skips only fully inactive tiles; a single active
  position triggers computation for that tile.
  (c)~Gather-scatter packs exactly the receptive fields of active positions, computes, and
  scatters results back.
  (d)~\sysname: \ding{192}~constructs a tile-local worklist and
  \ding{193}~consumes it via on-device adaptive Stream-$K$ execution,
  where data-parallel tiles complete a full reduction each while
  Stream-$K$ tail tiles split their reduction across SMs ($O_{14}^0$,
  $O_{14}^1$) and merge the partials.}
  \Description{Four-panel figure comparing masked-convolution execution
  paths. Panel~(a) shows the masked-convolution operation. Panel~(b)
  shows tile skipping with per-tile skip decisions. Panel~(c) shows
  gather-scatter with explicit extraction and writeback. Panel~(d) shows
  the \sysname two-stage pipeline: worklist construction from mask tiles
  and on-device adaptive Stream-$K$ execution with a data-parallel prefix
  and a Stream-$K$ tail that splits reduction across SMs.}
  \label{fig:overview}
\end{figure*}

\paragraph{Construction stage.}
To prevent worklist construction from erasing the savings of selective compute,
\sysname confines construction to mask and coordinate metadata
(Figure~\ref{fig:overview}d, \ding{192}), never reading feature or weight
tensors. In one mask-space pass, it fuses the output-mask propagation in
Eq.~\ref{eq:mask} with tile-local compaction, producing $M_{out}$ and a
worklist $\mathcal{W}$ (the figure shows the compaction step; mask propagation
precedes it but is omitted); the requested coordinates from each spatial tile
form a segment in local dense-traversal order, preserving spatial grouping
without global ordering.
\sysname writes $\mathcal{W}$ into a preallocated buffer large enough for the
full output grid. A device-resident counter records
$n_{\mathcal{W}}=|\mathcal{W}|$, so construction needs neither runtime
allocation nor a host-visible active count.

Because construction cost scales with the spatial grid rather than the active
count or channel widths, its share of latency grows as activity falls and
accumulates when every layer rebuilds. \sysname therefore
...
The compute stage (Figure~\ref{fig:overview}d, \ding{193}) replaces only the
coordinate source of a dense tiled implicit-GEMM pipeline with the worklist;
tensor layouts, weight access, and the per-position channel-and-kernel
reduction remain unchanged from the dense path.

The worklist length varies per frame with the input mask: the tile count is
unknown before launch and device-resident after construction. Synchronizing it
to the host for decomposition selection would insert a per-operator stall on
the critical path, yet a fixed decomposition loses efficiency across the
resulting range of tile counts. \sysname builds on Stream-$K$~\cite{streamk}, which addresses GPU
underutilization when tiles cannot fill all SMs but assumes the tile count is
host-visible at launch. \sysname designs
\emph{on-device adaptive Stream-$K$ execution}: it launches a persistent grid
sized to SM residency, and the kernel reads the device-resident worklist
length, looks up an offline-calibrated policy table, and selects its
decomposition per frame without host involvement. This sustains GPU throughput as the worklist length
