...
Exploiting dynamic spatial sparsity is a common way to accelerate convolutional
inference: an upstream policy emits a spatial mask, and the operator updates only
the marked positions. The mask comes from sources including event-camera
increments~\cite{asyncsparse,evconv}, temporal residuals between video
frames~\cite{cbinfer,skipconv,deltacnn,motiondelta,sparsetem}, learned input-dependent
gates~\cite{sact,sai,dgnet,dynconv}, etc. The event and temporal
...
194\,ms, far slower than dense convolution itself. That is due to their low effective throughput from different causes. Tile skipping holds
